...
abstract class Comment implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\MyPage\\Map\\CommentTableMap';
    ...
    /**
     * Initializes internal state of MyPage\Base\Comment object.
     */
    ...
    public function getCommentDate($format = NULL)
    {
        if ($format === null) {
            ...
            return $this->comment_date instanceof \DateTimeInterface ? $this->comment_date->format($format) : null;
        }
    }
    ...
    public function setAuthorName($v)
    {
        ...
        if ($this->author_name !== $v) {
            $this->author_name = $v;
            $this->modifiedColumns[CommentTableMap::COL_AUTHOR_NAME] = true;
        }
    ...
    public function setAuthorEmail($v)
    {
        ...
        if ($this->author_email !== $v) {
            $this->author_email = $v;
            $this->modifiedColumns[CommentTableMap::COL_AUTHOR_EMAIL] = true;
        }
     ...
     * @return $this|\MyPage\Comment The current object (for fluent API support)
     */
    public function setAvatarLink($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->avatar_link !== $v) {
            $this->avatar_link = $v;
            $this->modifiedColumns[CommentTableMap::COL_AVATAR_LINK] = true;
        }
    ...
    public function setCommentDate($v)
    {
        ...
        if ($this->comment_date !== null || $dt !== null) {
            if ($this->comment_date === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->comment_date->format("Y-m-d H:i:s.u")) {
                $this->comment_date = $dt === null ? null : clone $dt;
                $this->modifiedColumns[CommentTableMap::COL_COMMENT_DATE] = true;
            }
    ...
    public function setPageId($v)
    {
        ...
        if ($this->page_id !== $v) {
            $this->page_id = $v;
            $this->modifiedColumns[CommentTableMap::COL_PAGE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CommentTableMap::translateFieldName('AuthorName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->author_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CommentTableMap::translateFieldName('AuthorEmail', TableMap::TYPE_PHPNAME, $indexType)];
            $this->author_email = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CommentTableMap::translateFieldName('CommentTitle', TableMap::TYPE_PHPNAME, $indexType)];
            $this->comment_title = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CommentTableMap::translateFieldName('CommentText', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_AUTHOR_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'author_name';
        }
        if ($this->isColumnModified(CommentTableMap::COL_AUTHOR_EMAIL)) {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_COMMENT_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'comment_date';
        }
        if ($this->isColumnModified(CommentTableMap::COL_PAGE_ID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'avatar_link':
                        ...
                        break;
                    case 'comment_date':
                        $stmt->bindValue($identifier, $this->comment_date ? $this->comment_date->format("Y-m-d H:i:s.u") : null, PDO::PARAM_STR);
                        break;
                    case 'page_id':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array())
    {
        ...
        $result = array(
            ...
            $keys[3] => $this->getCommentTitle(),
            $keys[4] => $this->getCommentText(),
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setAuthorEmail($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setCommentTitle($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setCommentText($arr[$keys[4]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\MyPage\Comment The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_AUTHOR_NAME)) {
            $criteria->add(CommentTableMap::COL_AUTHOR_NAME, $this->author_name);
        }
        if ($this->isColumnModified(CommentTableMap::COL_AUTHOR_EMAIL)) {
            $criteria->add(CommentTableMap::COL_AUTHOR_EMAIL, $this->author_email);
        }
        if ($this->isColumnModified(CommentTableMap::COL_COMMENT_TITLE)) {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_COMMENT_DATE)) {
            $criteria->add(CommentTableMap::COL_COMMENT_DATE, $this->comment_date);
        }
        if ($this->isColumnModified(CommentTableMap::COL_PAGE_ID)) {
            $criteria->add(CommentTableMap::COL_PAGE_ID, $this->page_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MyPage\Comment (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \MyPage\Comment Clone of current object.
     * @throws PropelException
     */
